#    Martin Katkovcin @ Copyright 2022
#
...
class UDPHandler(socketserver.BaseRequestHandler):   
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar:
                del registrar[fromm]
                self.sendResponse("200 - Vsetko okay")
                return
        else:
            now = int(time.time())
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            logging.info(f'{origin.split("@")[0]} is calling {destination.split("@")[0]}')
            ...
            self.sendResponse("500 Server internal error (Custom)")
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("406 Neda sa prijat")
        else:
            self.sendResponse("500 Server Internal Error (Custom)")
